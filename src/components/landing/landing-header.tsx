...
export function LandingHeader() {
  ...
  return (
    <header className={`sticky top-0 z-40 border-b transition-all duration-300 ${scrolled ? 'border-orange-100 bg-white/90 shadow-[0_10px_30px_rgba(15,23,42,0.06)] glass' : 'border-transparent bg-white/70'}`}>
      <PageContainer constrained className="flex h-20 items-center justify-between gap-4">
        <Link to="/" aria-label="Vaija, página inicial" className="flex items-center gap-3">
        ...
        <nav className="hidden items-center gap-6 lg:flex">
          {nav.map((item) => <a key={item.href} href={item.href} className="rounded-full px-3 py-2 text-sm font-medium text-slate-600 transition-all duration-200 hover:bg-orange-50 hover:text-orange-600">{item.label}</a>)}
        </nav>
        <div className="hidden items-center gap-3 lg:flex">
          <Link to="/login"><Button variant="outline" className="border-orange-200 bg-white/90">Entrar</Button></Link>
          <Link to="/comprar?plano=Free"><Button className="shadow-[0_14px_30px_rgba(255,107,0,0.22)]">Começar grátis</Button></Link>
        </div>
        <div className="lg:hidden">
          <MobileDrawer trigger={<Button variant="outline" aria-label="Abrir menu" className="h-11 w-11 px-0"><Menu className="h-5 w-5" /></Button>}>
            <div className="flex h-full flex-col p-6">
              ...
              <div className="mt-6 flex flex-col gap-3">
                {nav.map((item) => <a key={item.href} href={item.href} className="rounded-2xl border border-orange-100 bg-white px-4 py-3 font-semibold text-slate-700 shadow-[0_8px_18px_rgba(15,23,42,0.04)]">{item.label}</a>)}
              </div>
              <div className="mt-auto grid gap-3 pt-6">
                <Link to="/login"><Button variant="outline" className="w-full border-orange-200 bg-white/90">Entrar</Button></Link>
                <Link to="/comprar?plano=Free"><Button className="w-full shadow-[0_14px_30px_rgba(255,107,0,0.22)]">Começar grátis</Button></Link>
              </div>
            </div>
          </MobileDrawer>
        </div>
